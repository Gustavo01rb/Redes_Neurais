...
#plot das amostras
plot_data(x, y, "Conjunto das amostras", "Agrupamento/results/amostras.png")
plot_data(x_train, y_train, "Conjunto de treinamento", "Agrupamento/results/train.png")
plot_data(x_test, y_test, "Conjunto de validação", "Agrupamento/results/test.png")


def unsupervised():
    model = C_Means(supervised=False)
    model.fit(x_train, n_centers= n_centers)
    model.info(path="Agrupamento/results/unsupervised_train/", x=x_train, show=False)
    Y, erro, accuracy = model.predict_group(x_test, y_test)
    plot_data(x_test, Y, "Validação não supervisionado", "Agrupamento/results/unsupervised_test/result.png")
    print("\nResultados obtidos: ")
...
def supervised():
    ...
    model.info(path="Agrupamento/results/supervised_train/"  , x=x_train, show=False)
    Y, erro, accuracy = model.predict_group(x_test, y_test)
    plot_data(x_test, Y, "Validação supervisionado", "Agrupamento/results/supervised_test/result.png")
    print("\nResultados obtidos: ")
    print("\tNúmero de erros: ", erro)
    print(f"\tPorcentagem de acerto: {round(accuracy * 100, 2)} %")
